...
public class FT_TravellerDetailsPage {
    ...
    public void enterInvalidAdult() {
        try {
            ExcelReader.setExcelFile(path, "Adult");
            ...
            type(firstName, ExcelReader.getCellData(1, 0)); // invalid
            type(lastName, ""); // 🔥 MUST be empty

            selectGender(ExcelReader.getCellData(1, 2));
    ...
    public void enterValidAdult() {
        try {
            ...
            type(firstName, ExcelReader.getCellData(3, 0));
            type(lastName, ExcelReader.getCellData(3, 1));

            selectGender(ExcelReader.getCellData(3, 2));
    ...
    public void enterInvalidChild() {
        try {
            ExcelReader.setExcelFile(path, "Child");
            ...
            String fname = ExcelReader.getCellData(1, 0);
            ...
            new Select(wait.until(ExpectedConditions.elementToBeClickable(childGender)))
                    .selectByVisibleText(ExcelReader.getCellData(1, 2));
    ...
    public void enterValidChild() {
        try {
            ...
            type(childFirst, ExcelReader.getCellData(1, 0));
            type(childLast, ExcelReader.getCellData(1, 1));

            new Select(wait.until(ExpectedConditions.elementToBeClickable(childGender)))
                    .selectByVisibleText(ExcelReader.getCellData(1, 2));

            new Select(driver.findElement(childMonth))
                    .selectByVisibleText(ExcelReader.getCellData(1, 3));

            type(childDay, ExcelReader.getCellData(1, 4));
            type(childYear, ExcelReader.getCellData(1, 5));
    ...
    public void enterValidContact() {
        clear(email);
        type(email, ConfigReader.get("email"));
