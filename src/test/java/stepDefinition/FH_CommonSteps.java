...
public class FH_CommonSteps {
    ...
    public void user_on_homepage_after_login() {
        searchPage = new FH_FlightsHotelsSearchPage(DriverFactory.getDriver());
    }
    ...
    public void enter_departure(int row) {
        String dep = ExcelReader.getAllData().get(row - 1).get("Departure");
    ...
    public void enter_destination(int row) {
        String dest = ExcelReader.getAllData().get(row - 1).get("Destination");
